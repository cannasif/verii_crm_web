...
export function DemandSummaryCard({
  ...
  const watchedRate = form.watch('demand.generalDiscountRate');
  const watchedAmount = form.watch('demand.generalDiscountAmount');
  ...
  const handleRateChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (raw === '' || raw === undefined) {
        form.setValue('demand.generalDiscountRate', null, { shouldValidate: true });
        form.setValue('demand.generalDiscountAmount', null, { shouldValidate: true });
        return;
      }
      ...
      form.setValue('demand.generalDiscountRate', round2(rate), { shouldValidate: true });
      const amount = netTotal > 0 ? round2(Math.min(netTotal * (rate / 100), netTotal)) : 0;
      form.setValue('demand.generalDiscountAmount', amount, { shouldValidate: true });
  ...
  const handleAmountChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      form.setValue('demand.generalDiscountRate', rate, { shouldValidate: true });
      skipSyncRef.current = null;
    },
    [form, netTotal]
  );
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white/95 dark:bg-zinc-900/50 shadow-sm">
      <div className="p-5 sm:p-6">
        <div className="flex items-center gap-2.5 mb-6 text-zinc-900 dark:text-white">
          ...
          <h3 className="font-bold text-sm tracking-tight">{t('demand.summary.title', 'Özet')}</h3>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-4 sm:gap-y-0 sm:items-start">
          <div className="flex flex-col gap-1.5 min-w-0">
            ...
            >
              {t('demand.summary.generalDiscountRate', 'Genel İskonto Oranı (%)')}
            </Label>
            <div className="relative">
              <Input
                id="demand-generalDiscountRate"
                type="number"
                min={0}
                max={100}
                step={0.01}
                ...
                aria-invalid={!!form.formState.errors.demand?.generalDiscountRate}
              />
          ...
          <div className="flex flex-col gap-1.5 min-w-0">
            <Label
              ...
              className="text-sm font-medium text-zinc-700 dark:text-zinc-300 leading-tight"
            >
              {t('demand.summary.generalDiscountAmount', 'Genel İskonto Tutarı')}
            </Label>
            <Input
              ...
              aria-invalid={!!form.formState.errors.demand?.generalDiscountAmount}
            />
          </div>
        </div>
        ...
        <p className="mt-3 pl-3 border-l-2 border-zinc-200 dark:border-zinc-700 text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed">
          {t('demand.summary.generalDiscountHelp', 'Oran veya tutar girildiğinde toplamlar otomatik güncellenir.')}
        </p>
          ...
          <dl className="space-y-3.5">
            <div className="flex items-center justify-between gap-3 min-w-0">
              <dt className="text-sm text-zinc-500 dark:text-zinc-400 font-medium truncate">
                {t('demand.summary.subtotal', 'Ara Toplam')}
              </dt>
            ...
            {totals.generalDiscountAmount > 0 && (
              <div className="flex items-center justify-between gap-3 min-w-0">
                <dt className="text-sm text-zinc-500 dark:text-zinc-400 font-medium truncate">
                  {t('demand.summary.generalDiscount', 'Genel İskonto')}
                </dt>
            ...
            <div className="flex items-center justify-between gap-3 min-w-0">
              <dt className="text-sm text-zinc-500 dark:text-zinc-400 font-medium truncate">
                {t('demand.summary.totalVat', 'Toplam KDV')}
              </dt>
              <dd className="font-semibold text-zinc-900 dark:text-zinc-100 font-mono tabular-nums text-right text-sm shrink-0 min-w-24">
        ...
        <div className="mt-4 pt-4 border-t-2 border-zinc-200 dark:border-zinc-700">
          <div className="flex items-center justify-between gap-3 min-w-0">
            <span className="text-zinc-700 dark:text-zinc-300 font-semibold text-sm shrink-0 flex items-center gap-2 truncate">
              ...
              {t('demand.summary.grandTotal', 'Genel Toplam')}
            </span>
