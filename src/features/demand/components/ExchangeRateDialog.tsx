...
export function ExchangeRateDialog({
  ...
  const mapRatesToUpdateDtos = useCallback(
    (rates: DemandExchangeRateFormState[]): DemandExchangeRateGetDto[] =>
      rates.map((r) => ({
        id: parseRateId(r.id),
        demandId: demandId ?? 0,
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto p-0 gap-0 overflow-hidden bg-white/95 dark:bg-zinc-950/95 backdrop-blur-xl border-zinc-200 dark:border-zinc-800 shadow-2xl">
        ...
        <DialogHeader className="p-6 pb-4 border-b border-zinc-200/50 dark:border-zinc-800/50 bg-zinc-50/30 dark:bg-zinc-900/10">
          <DialogTitle className="flex items-center gap-3 text-xl">
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <span className="text-xs font-normal text-muted-foreground">{t('exchangeRates.dialog.subtitle')}</span>
            </div>
          </DialogTitle>
        ...
        <div className="p-6">
              ...
              <Table>
                <TableHeader>
                  <TableRow className="hover:bg-transparent border-0">
                    ...
                    <TableHead className={cn(styles.tableHead, "text-center")}>{t('exchangeRates.status')}</TableHead>
                    <TableHead className={cn(styles.tableHead, "text-center w-[120px]")}>{t('actions')}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {localRates.length === 0 ? (
          ...
          <div className="mt-4 bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 rounded-lg p-3 text-xs text-blue-600 dark:text-blue-400 flex gap-2 items-start">
             ...
             <p>{t('exchangeRates.dialog.info')}</p>
          </div>
        </div>
