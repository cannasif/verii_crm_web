...
const getColumnsConfig = (t: TFunction): ColumnDef<ActivityTypeDto>[] => [
    { key: 'id', label: t('activityType.table.id'), type: 'text', className: 'font-medium w-[80px]' },
    { key: 'name', label: t('activityType.table.name'), type: 'text', className: 'font-semibold text-slate-900 dark:text-white min-w-[200px]' },
    { key: 'description', label: t('activityType.table.description'), type: 'description', className: 'min-w-[250px]' },
    { key: 'createdDate', label: t('activityType.table.createdDate'), type: 'date', className: 'whitespace-nowrap' },
    { key: 'createdByFullUser', label: t('activityType.table.createdBy'), type: 'user', className: 'whitespace-nowrap' },
];
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between py-4 gap-4">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage <= 1} className="bg-white dark:bg-transparent border-slate-200 dark:border-white/10 hover:bg-slate-50 dark:hover:bg-white/5">{t('activityType.previous')}</Button>
          <div className="flex items-center px-4 text-sm font-medium text-slate-700 dark:text-slate-200">{t('activityType.table.page', { current: currentPage, total: totalPages || 1 })}</div>
          <Button variant="outline" size="sm" onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage >= totalPages} className="bg-white dark:bg-transparent border-slate-200 dark:border-white/10 hover:bg-slate-50 dark:hover:bg-white/5">{t('activityType.next')}</Button>
        </div>
      </div>
        ...
        <DialogContent className="bg-white dark:bg-[#130822] border border-slate-100 dark:border-white/10 text-slate-900 dark:text-white w-[90%] sm:w-full max-w-md rounded-2xl shadow-2xl overflow-hidden p-0 gap-0">
          ...
          <DialogHeader className="flex flex-col items-center gap-4 text-center pb-6 pt-10 px-6">
            ...
            <div className="space-y-2">
                ...
                {t('activityType.delete.confirmTitle')}
                </DialogTitle>
                <DialogDescription className="text-slate-500 dark:text-slate-400 max-w-[280px] mx-auto text-sm leading-relaxed">
                {t('activityType.delete.confirmMessage', {
