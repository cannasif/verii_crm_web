...
export function QuotationLineForm({
  ...
  const hideVatRate = effectiveSystemSettings.hideQuotationVatRate;
  const readonlyVatRate = effectiveSystemSettings.readonlyQuotationVatRate;
    ...
    <div className="space-y-6 animate-in fade-in zoom-in-95 duration-300">
      <div className="space-y-4">
        <label className="text-sm font-semibold text-slate-500 dark:text-slate-400 flex items-center gap-2">
          ...
          {t('lines.stock')}
          {isZodFieldRequired(quotationLineRequiredSchema, 'productCode') ? <span className="text-rose-500">*</span> : null}
        </label>

        <div className="flex flex-col gap-3">
          <div className="flex flex-row gap-3">
            <div className="group relative min-w-0 flex-1">
              <div className="pointer-events-none absolute left-3 top-1/2 z-20 flex -translate-y-1/2 items-center justify-center text-zinc-400 transition-colors group-focus-within:text-rose-500">
