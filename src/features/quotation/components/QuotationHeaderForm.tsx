...
import { QuotationStructuredNotesList } from './QuotationStructuredNotesList';
import { QuotationNotesAddLineButton } from './QuotationNotesAddLineButton';
...
export function QuotationHeaderForm({
  ...
  const specialCode2DefaultExists = useSpecialCodeExists(2, defaultSpecialCode, !readOnly && !quotationId);
  ...
  const handleComboboxSelect = (option: CustomerComboboxOption): void => {
    ...
    if (option.erpCurrencyType != null) {
      form.setValue('quotation.currency', String(option.erpCurrencyType), { shouldDirty: true, shouldValidate: true });
    }
    if (option.paymentTermDays != null) {
      form.setValue('quotation.paymentTermDays', option.paymentTermDays, { shouldDirty: true, shouldValidate: true });
    }
          ...
          <div className="p-5 h-full flex flex-col">
            ...
            <div className="space-y-4 flex-1">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="space-y-0 relative group">
                    <FormLabel className={styles.label}>{t('quotation:header.paymentTermDays', { defaultValue: 'Vade Günü' })}</FormLabel>
                    <div className="relative">
           ...
           <div className="p-5">
              ...
              <div className="space-y-4">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="gap-0 space-y-0 relative group w-full min-w-0">
                      <FormLabel className={styles.label} required={isZodFieldRequired(createQuotationSchema, 'quotation.ozelKod1')}>
                        <Layers className="h-3.5 w-3.5" />
                        {t('quotation:header.ozelKod1')}
                      </FormLabel>
                      <div className="relative w-full min-w-0">
                        <div className={cn(styles.iconWrapper, getIconTone(Boolean(field.value)))}><Layers className="h-4 w-4" /></div>
                        <FormControl>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="gap-0 space-y-0 relative group w-full min-w-0">
                      <FormLabel className={styles.label} required={isZodFieldRequired(createQuotationSchema, 'quotation.ozelKod2')}>
                        <Layers className="h-3.5 w-3.5" />
                        {t('quotation:header.ozelKod2')}
                      </FormLabel>
                      <div className="relative w-full min-w-0">
                        <div className={cn(styles.iconWrapper, getIconTone(Boolean(field.value)))}><Layers className="h-4 w-4" /></div>
                        <FormControl>
