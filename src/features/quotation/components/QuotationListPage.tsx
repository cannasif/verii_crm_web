...
const QUOTATION_SEARCH_FIELD_CONFIG = [
  { key: 'OfferNo', labelKey: 'quotation.list.offerNo', fallbackLabel: 'Teklif No' },
  { key: 'RevisionNo', labelKey: 'quotation.list.revisionNo', fallbackLabel: 'Revize No' },
  { key: 'PotentialCustomer.CustomerName', labelKey: 'quotation.list.customer', fallbackLabel: 'Müşteri' },
  { key: 'PotentialCustomer.CustomerCode', labelKey: 'quotation.list.customerCode', fallbackLabel: 'Cari Kodu' },
  { key: 'ErpCustomerCode', labelKey: 'quotation.list.customerCode', fallbackLabel: 'ERP Cari Kodu' },
  { key: 'Representative.FirstName', labelKey: 'quotation.list.representative', fallbackLabel: 'Temsilci adı' },
  { key: 'Representative.LastName', labelKey: 'quotation.list.representative', fallbackLabel: 'Temsilci soyadı' },
  { key: 'KoliBaskiDefinition.Name', labelKey: 'quotation.list.koliBaski', fallbackLabel: 'Koli Baskı' },
  { key: 'Currency', labelKey: 'quotation.list.currency', fallbackLabel: 'Para Birimi' },
  { key: 'ERPIntegrationNumber', labelKey: 'quotation.list.erpIntegrationNumber', fallbackLabel: 'Netsis No' },
] as const;
...
export function QuotationListPage(): ReactElement {
  ...
  const searchableColumns = useMemo(() => QUOTATION_SEARCH_FIELD_CONFIG.map((field) => field.key), []);
  const searchFieldOptions = useMemo(
    () => QUOTATION_SEARCH_FIELD_CONFIG.map((field) => ({
      key: field.key,
  ...
  }, [exportColumns, searchTerm, searchFields, sortBy, sortDirection, filtersParam, approvalStatusFilter, i18n.language, getCurrencyLabel, getGrandTotalLabel, getErpIntegrationExportLabel, getErpDocumentNumber, getApprovalStatusLabel]);
