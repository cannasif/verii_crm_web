...
import type { ActivityImageDto, UpdateActivityImageDto } from '../types/activity-image-types';
...
export function useUpdateActivityImage(activityId: number) {
  ...
  return useMutation({
    ...
    onSuccess: async (updatedImage) => {
      ...
      await queryClient.invalidateQueries({ queryKey: activityImageKeys.byActivity(activityId) });
