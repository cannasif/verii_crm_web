...
import { Plus, Search, RefreshCw, X, Users, Edit2, Trash2, Loader2, FileSearch } from 'lucide-react';
...
export function GroupList(): ReactElement {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter(
      (x) => normalizeSearchValue(x.name).includes(lower) || normalizeSearchValue(x.description).includes(lower)
    );
  }, [items, searchTerm]);
          ...
          <Table>
            <TableHeader>
              <TableRow className="font-bold text-slate-700 dark:text-white dark:bg-[#231A2C] border-b border-slate-200 dark:border-white/5">
                <TableHead className="text-xs font-bold uppercase tracking-wider text-slate-900 dark:text-white py-4 border-r border-slate-200 dark:border-white/5">{t('powerbi.group.name')}</TableHead>
                <TableHead className="text-xs font-bold uppercase tracking-wider text-slate-900 dark:text-white border-r border-slate-200 dark:border-white/5">{t('powerbi.group.description')}</TableHead>
                <TableHead className="text-xs font-bold uppercase tracking-wider text-slate-900 dark:text-white border-r border-slate-200 dark:border-white/5">{t('powerbi.group.isActive')}</TableHead>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <DialogDescription className="text-sm font-medium text-slate-500 dark:text-slate-400 mt-1">
                  {t('common.delete.confirmMessage', { name: selectedItem?.name ?? '' })}
                </DialogDescription>
              </div>
            </div>
          </DialogHeader>
