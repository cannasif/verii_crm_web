...
export function SalesWorkFeedTab({
  ...
  const [kind, setKind] = useState<(typeof WORK_KIND_FILTERS)[number]>('all');
    ...
    <TabsContent value="work" className="space-y-5 outline-none">
        ...
        <CardHeader className="gap-4 pb-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <CardTitle className="text-base">
              {t('salesman360.performance.detail.work.title')}
            </CardTitle>
            <p className="mt-1 text-xs font-medium text-slate-500">
              {t('salesman360.performance.detail.work.description')}
            </p>
          </div>
              ...
              <TableHeader>
                <TableRow className="bg-slate-50/80 dark:bg-white/3">
                  ...
                  <TableHead className="text-right">
                    {t('salesman360.performance.detail.work.amount')}
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                      ...
                      return (
                        ...
                        >
                          <TableCell>
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                ...
                                {item.typeName ? (
                                  <p className="text-[11px] text-slate-400">{item.typeName}</p>
                                ) : null}
                              </div>
                              {route ? (
                          ...
                          <TableCell className="whitespace-nowrap text-xs">
                            {formatPerformanceDate(item.date, locale)}
                          </TableCell>
                          <TableCell className="text-right font-bold tabular-nums">
                            {item.amount != null && item.currency
                              ? formatPerformanceAmount(item.amount, item.currency, locale)
                              : '-'}
                          </TableCell>
                        </TableRow>
                      );
      ...
      <Card className="rounded-2xl border-slate-200/90 bg-white/90 shadow-sm dark:border-white/10 dark:bg-white/3">
        <CardHeader className="pb-2">
          <CardTitle className="text-base">
            {t('salesman360.performance.detail.attention.allTitle')}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <PerformanceAttentionTable items={attentionItems} locale={locale} />
        </CardContent>
      </Card>
    </TabsContent>
  );
