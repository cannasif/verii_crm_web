...
export function SystemSettingsForm({
  ...
  const headerCardStyle = `
    w-full min-w-0 max-w-full overflow-hidden rounded-[2rem] border border-slate-200 dark:border-white/10
    bg-white dark:bg-[#180F22] backdrop-blur-md px-1 py-4 sm:px-2 sm:py-6
    shadow-[0_8px_24px_-16px_rgba(15,23,42,0.28)] dark:shadow-[0_8px_24px_-16px_rgba(0,0,0,0.55)]
    transition-all duration-300 relative
  `;

  const tabTriggerClassName = [
    'h-auto flex-1 basis-[12rem] min-w-[min(100%,12rem)] justify-center rounded-xl border-2 border-slate-300',
    'bg-white px-3 py-2.5 text-center text-sm font-semibold whitespace-nowrap text-slate-600 shadow-none',
    'transition-colors hover:border-primary/50',
    'data-[state=active]:border-primary data-[state=active]:bg-accent data-[state=active]:text-primary data-[state=active]:shadow-sm',
    'dark:border-white/25 dark:bg-[#0C0516] dark:text-slate-300 dark:hover:border-primary/50',
    'dark:data-[state=active]:border-primary dark:data-[state=active]:bg-accent/20 dark:data-[state=active]:text-primary',
  ].join(' ');

  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="w-full min-w-0 max-w-full space-y-6" noValidate>
        <Tabs defaultValue="general" className="w-full min-w-0 max-w-full gap-4">
          <TabsList className="flex h-auto w-full min-w-0 flex-wrap items-stretch justify-start gap-2 bg-transparent p-0">
            <TabsTrigger value="general" className={tabTriggerClassName}>
              {t('systemSettings.Tabs.General', 'Genel')}
            </TabsTrigger>
            <TabsTrigger value="vat-settings" className={tabTriggerClassName}>
              {t('systemSettings.Tabs.VatSettings', 'KDV oranı ayarları')}
            </TabsTrigger>
            <TabsTrigger value="code-format-rules" className={tabTriggerClassName}>
              {t('systemSettings.Tabs.CodeFormatRules', 'Kod formatı kuralları')}
            </TabsTrigger>
            <TabsTrigger value="document-field-labels" className={tabTriggerClassName}>
              {t('systemSettings.Tabs.DocumentFieldLabels', 'Belge alanı başlıkları')}
            </TabsTrigger>
          </TabsList>

          <Card className={headerCardStyle}>
            <CardContent className="min-w-0 space-y-6 overflow-hidden px-3 sm:px-6">
              <TabsContent value="general" forceMount className="mt-0 min-w-0 data-[state=inactive]:hidden">
                <div className="grid w-full min-w-0 grid-cols-1 gap-4 md:grid-cols-2">
                  <div className="min-w-0 rounded-2xl border border-slate-200 bg-slate-50 p-3 sm:p-4 dark:border-white/10 dark:bg-white/[0.03] md:col-span-2">
                    <div className="flex min-w-0 flex-col gap-4 md:flex-row md:items-center md:justify-between">
                      <div className="flex min-w-0 items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1 space-y-1 overflow-hidden">
                          <p className="break-words text-sm font-semibold">{t('systemSettings.ErpConnection.Title')}</p>
                          <p className="text-muted-foreground break-words text-sm">{t('systemSettings.ErpConnection.Description')}</p>
                          {erpConnectionMessage ? (
                            <div className="mt-2 flex min-w-0 items-start gap-2 text-sm">
                              ...
                              <div className={`min-w-0 break-words ${erpConnectionSucceeded ? 'text-emerald-700 dark:text-emerald-300' : 'text-red-700 dark:text-red-300'}`}>
                                <p className="break-words">{erpConnectionMessage}</p>
                      ...
                      <Button
                        ...
                        className="h-auto w-full shrink-0 whitespace-normal rounded-xl px-4 py-2.5 text-left md:w-auto"
                      >
                        {isTestingErpConnection ? (
                          <>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="min-w-0 md:col-span-2">
                        <div className="flex min-w-0 items-start gap-3 rounded-xl border border-sky-200 bg-sky-50/80 p-3 sm:p-4 dark:border-sky-400/20 dark:bg-sky-400/10">
                          <FormControl>
                            <Checkbox
                          ...
                          <div className="min-w-0 flex-1 space-y-1 overflow-hidden">
                            <FormLabel required={false} className="break-words">
                              {t('systemSettings.Fields.UseCurrentDateForErpRegistration', 'ERP kayit tarihi islem tarihi olsun')}
                            </FormLabel>
                            <p className="text-muted-foreground break-words text-sm">
                              {t('systemSettings.Descriptions.UseCurrentDateForErpRegistration', 'Acikken Netsis kayit tarihi ERP gonderim anidir. Kapaliyken talep, teklif veya siparisin kendi belge tarihi kullanilir. Fiyat ve kur tarihi degismez.')}
                            </p>
                          </div>
                        </div>
                        <FormMessage />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="min-w-0">
                        ...
                        <FormControl>
                          <Input
                            className="w-full min-w-0 rounded-2xl border border-slate-200 bg-white dark:border-white/10 dark:bg-[#0C0516]"
                            type="number"
                            min={0}
                            max={6}
                            step={1}
                  ...
                  <div className="min-w-0 rounded-2xl border border-slate-200 bg-slate-50 p-3 sm:p-4 dark:border-white/10 dark:bg-white/[0.03] md:col-span-2">
                    <div className="mb-3 min-w-0 space-y-1">
                      <p className="break-words text-sm font-semibold">{t('systemSettings.Sections.CatalogLabels', 'Katalog kod alanı isimleri')}</p>
                      <p className="text-muted-foreground break-words text-sm">
                  ...
                  <FormField
                    ...
                    render={() => {
                      ...
                      return (
                        <FormItem className="min-w-0">
                          <FormLabel className="break-words">{t('systemSettings.Fields.DemandApprovalCompletionAction')}</FormLabel>
                          <Select
                            value={selectedValue}
                            onValueChange={(value) => {
                  ...
                  <FormField
                    ...
                    render={() => {
                      ...
                      return (
                        <FormItem className="min-w-0">
                          <FormLabel className="break-words">{t('systemSettings.Fields.QuotationApprovalCompletionAction')}</FormLabel>
                          <Select
                            value={selectedValue}
                            onValueChange={(value) => {
                              const nextValue = Number(value);
                  ...
                  <FormField
                    ...
                    render={() => {
                      ...
                      return (
                        <FormItem className="min-w-0">
                          <FormLabel className="break-words">{t('systemSettings.Fields.OrderApprovalCompletionAction')}</FormLabel>
                          <Select
                            value={selectedValue}
                            onValueChange={(value) => {
                              const nextValue = Number(value);
              ...
              <TabsContent value="vat-settings" forceMount className="mt-0 data-[state=inactive]:hidden">
              ...
              <TabsContent value="document-field-labels" forceMount className="mt-0 min-w-0 data-[state=inactive]:hidden">
                <DocumentFieldLabelsSettingsPanel onItemsChange={handleDocumentFieldLabelsChange} />
              </TabsContent>

              <TabsContent value="code-format-rules" forceMount className="mt-0 data-[state=inactive]:hidden">
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/[0.03]">
                  <div className="mb-3 space-y-1">
