...
const TABLE_COLUMNS: Array<{ key: SalesRepColumnKey; labelKey: string; className?: string }> = [
  { key: 'branchCode', labelKey: 'table.branchCode', className: 'w-[120px]' },
  { key: 'salesRepCode', labelKey: 'table.salesRepCode', className: 'w-[160px] font-medium' },
  { key: 'salesRepDescription', labelKey: 'table.salesRepDescription', className: 'min-w-[220px]' },
  { key: 'name', labelKey: 'table.name', className: 'min-w-[220px]' },
...
export function SalesRepManagementPage(): ReactElement {
  ...
  const [sortBy, setSortBy] = useState<SalesRepColumnKey>('salesRepCode');
  ...
  const { data: apiResponse, isLoading, isFetching } = useSalesRepList({
    pageNumber,
    pageSize,
  ...
  const orderedVisibleColumns = columnOrder.filter((key) => visibleColumns.includes(key)) as SalesRepColumnKey[];

  const columns = useMemo<DataTableGridColumn<SalesRepColumnKey>[]>(
    () =>
  ...
  const filterColumns = useMemo(
    () =>
      SALES_REP_FILTER_COLUMNS.map((column) => ({
        value: column.value,
        type: column.type,
  ...
  const mapRow = useCallback(
    (item: SalesRepGetDto): Record<string, unknown> => {
  ...
  return (
    <div className="w-full space-y-6 relative">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pt-2">
        ...
        <Button onClick={() => setFormOpen(true)} className="px-6 py-2 bg-linear-to-r from-sky-600 to-cyan-600 rounded-xl text-white text-sm font-bold shadow-lg shadow-sky-500/20 hover:scale-105 transition-transform border-0 hover:text-white h-11">
      ...
      <Card className={MANAGEMENT_LIST_CARD_CLASSNAME}>
        <CardHeader className={MANAGEMENT_LIST_CARD_HEADER_CLASSNAME}>
          ...
          <DataTableActionBar
            ...
            translationNamespace="sales-rep-management"
