...
import {
  useOutlookAuthorizeMutation,
  useOutlookDisconnectMutation,
} from '../hooks/useOutlookIntegrationMutations';
...
export function OutlookConnectionPage(): ReactElement {
  ...
  const { data: status, isLoading, isFetching } = useOutlookStatusQuery();
  const authorizeMutation = useOutlookAuthorizeMutation();
  const disconnectMutation = useOutlookDisconnectMutation();
  ...
  useEffect(() => {
    ...
    navigate('/settings/integrations/outlook', { replace: true });
  ...
  return (
    <div className="w-full space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">{t('page.connectionDescription')}</p>
      </div>
          ...
          {!isLoading && (
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5 text-sm">
                <div>
                  <p className="text-muted-foreground mb-1.5">{t('connection.outlookEmailLabel')}</p>
                  <div className="rounded-lg border border-slate-200 bg-slate-50 dark:border-[#3b3142] dark:bg-[#1e1627] px-3 py-2">
                    <p className="font-medium break-words">{status?.outlookEmail || '-'}</p>
                  </div>
                </div>
                <div>
