...
export function useCustomerActivities(customerId?: number | null, search = '', enabled = true) {
  ...
  return useQuery<ActivityDto[]>({
    ...
    enabled: enabled && Boolean(normalizedCustomerId),
    queryFn: async () => {
      const result = await activityApi.getList({
        pageNumber: 1,
        pageSize: 50,
        search,
        searchFields: search ? ['Subject'] : undefined,
