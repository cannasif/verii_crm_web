...
export type OrderQuickEditField =
  | 'quantity'
  | 'unitPrice'
  | 'discountRate1'
  | 'discountRate2'
...
export interface ApplyOrderLineQuickFieldPatchDeps {
  currency: number;
  currencyOptions: CurrencyOption[];
  exchangeRates: OrderExchangeRateFormState[];
  erpRates: KurDto[];
...
/** Applies one field change with the same pricing-tier + discount-limit rules as OrderLineForm (without temporaryStockData fallback). */
export function applyOrderLineQuickFieldPatch(
  line: OrderLineFormState,
  field: OrderQuickEditField,
  value: number,
  deps: ApplyOrderLineQuickFieldPatchDeps
): OrderLineFormState {
  const { currency, currencyOptions, exchangeRates, erpRates, pricingRules, userDiscountLimits, calculateLineTotals } =
    deps;

  let next: OrderLineFormState = { ...line, [field]: value };
