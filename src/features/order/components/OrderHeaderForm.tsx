...
export function OrderHeaderForm({
  ...
  const watchedShippingAddressId = form.watch('order.shippingAddressId');
  const watchedActivityId = form.watch('order.activityId');
  ...
  const { data: relatedUsers = [], isLoading: isRelatedUsersLoading } = useOrderRelatedUsers(user?.id);
  const {
  ...
  useEffect(() => {
    if (!watchedCustomerId) {
      if (watchedActivityId != null) {
        form.setValue('order.activityId', null, { shouldDirty: true, shouldValidate: true });
      }
      return;
    }
              ...
              <div className="xl:col-span-2 space-y-2">
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1 group min-w-0">
                    <div className={cn("absolute left-3 top-1/2 -translate-y-1/2 z-10 pointer-events-none transition-colors", customerSearchQuery?.trim() ? "text-primary" : "text-zinc-400 group-focus-within:text-primary")}>
              ...
              <div className="animate-in fade-in slide-in-from-top-2 duration-500">
                <div className="space-y-2">
                  <div className={styles.label}>
                    ...
                    {t('order:header.activity', { defaultValue: 'Bağlı Aktivite' })}
                  </div>
                  <FormField
                    control={form.control}
                    name="order.activityId"
                    render={({ field }) => (
                      <FormItem className="space-y-0 min-w-0">
                        <FormControl>
                          <VoiceSearchCombobox
                            ...
                            placeholder={t('order:header.selectActivity', { defaultValue: 'Aktivite seçin' })}
