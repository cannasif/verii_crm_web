...
export function OrderLineForm({
  ...
  const hideVatRate = effectiveSystemSettings.hideOrderVatRate;
  const readonlyVatRate = effectiveSystemSettings.readonlyOrderVatRate;
  ...
  const handleFieldChangeRef = useRef<(field: keyof OrderLineFormState, value: unknown) => void>(() => { });
  ...
  const getDefaultBaskiId = useCallback((): number | null => {
    const isExistingDoc = !!imageUploadExtras?.orderId;
  ...
  }, [baskiOptions, line.baskiDefinitionId, imageUploadExtras?.orderId]);
  ...
  return (
    <div className="space-y-6 animate-in fade-in zoom-in-95 duration-300">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  maxLength={200}
                  placeholder={t('order.lines.lineItemDescriptionPlaceholder', { defaultValue: 'Satır bazlı kalem açıklaması' })}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-3">
              <div className="space-y-1.5">
                <div className="flex items-center gap-1.5 ml-1">
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-200">
                    {t('order.lines.windoProfileLabel')} <span className="text-primary">*</span>
                  </label>
                ...
                <VoiceSearchCombobox
                  ...
                  placeholder={profilDropdown.isLoading ? t('order.loading') : t('order.lines.selectWindoProfile')}
                ...
                >
                  ...
                  {t('order.lines.addNewProfile', { defaultValue: 'Yeni profil ekle' })}
                </Button>
              </div>
              <div className="space-y-1.5">
                <div className="flex items-center gap-1.5 ml-1">
                ...
                <VoiceSearchCombobox
                  ...
                  placeholder={demirDropdown.isLoading ? t('order.loading') : t('order.lines.selectWindoRebar')}
                ...
                >
                  <CirclePlus className="mr-1 h-3.5 w-3.5" />
                  {t('order.lines.addNewRebar', { defaultValue: 'Yeni demir ekle' })}
                </Button>
              </div>
              <div className="space-y-1.5">
                <div className="flex items-center gap-1.5 ml-1">
                ...
                <VoiceSearchCombobox
                  ...
                  placeholder={vidaDropdown.isLoading ? t('order.loading') : t('order.lines.selectWindoScrew')}
              ...
              <div className="space-y-1.5">
                ...
                <VoiceSearchCombobox
                  ...
                  placeholder={baskiDropdown.isLoading ? t('order.loading') : t('order.lines.selectPrint', { defaultValue: 'Baskı seçin' })}
                ...
                <Input
                  ...
                  maxLength={50}
                  placeholder={t('order.lines.windoPrintDescriptionPlaceholder', { defaultValue: 'Baskı açıklaması (max 50 karakter)' })}
                  className={`h-11 rounded-xl border-slate-200 bg-slate-50 text-slate-900 dark:border-white/10 dark:bg-white/[0.04] dark:text-white ${pinkFocusClass}`}
                />
                <Button
                  type="button"
            ...
            {(() => {
              ...
              if (bulkDraftLines.length === 0) {
                if (!formData.productCode || !formData.productName) {
                  missingFields.push(t('order.lines.stockSelectionRequired', { defaultValue: 'Stok Seçimi' }));
                }
                if (!formData.profilDefinitionId) {
                  missingFields.push(t('order.lines.windoProfileLabel', { defaultValue: 'Profil' }));
                }
              } else {
      ...
      <CatalogStockSelectDialog
        ...
        onMultiSelect={handleMultiProductSelect}
        existingLineStockMarkers={existingLineStockMarkers}
        pricingRuleType={PricingRuleType.Order}
        initialSelectedResults={bulkDraftLines.map((lineItem) => ({
          ...(lineItem.productId != null && lineItem.productId > 0 ? { id: lineItem.productId } : {}),
          code: lineItem.productCode || '',
          name: lineItem.productName || '',
