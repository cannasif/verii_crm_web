...
export function ExchangeRateDialog({
  ...
  const findRateForCurrency = useCallback(
    (rates: OrderExchangeRateFormState[]): OrderExchangeRateFormState | undefined => {
  ...
  const applyCreateModeLineRepricing = useCallback(
    (ratesToUse: OrderExchangeRateFormState[]): void => {
  ...
  useEffect(() => {
    ...
    setEditingDraft('');
    setEditMountKey(0);
    const mappedRates: OrderExchangeRateFormState[] = erpRates.map((rate, index) => {
  ...
  const mapRatesToUpdateDtos = useCallback(
    (rates: OrderExchangeRateFormState[]): OrderExchangeRateGetDto[] =>
      rates.map((r) => ({
        id: parseRateId(r.id),
        orderId: orderId ?? 0,
              ...
              <Table>
                <TableHeader>
                  <TableRow className="hover:bg-transparent border-0">
                    ...
                    <TableHead className={cn(styles.tableHead, "text-center w-[120px]")}>{t('order.actions')}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {localRates.length === 0 ? (
                          ...
                          <TableCell className="text-center">
                            {editingId === rate.id ? (
                              ...
                              <Button
                                ...
                                className="h-8 gap-1.5 px-2 text-zinc-500 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20"
                                title={t('order.edit')}
                              >
                                <Edit2 className="h-4 w-4" />
                                <span className="text-xs font-medium">{t('order.edit')}</span>
                              </Button>
                            )}
                          </TableCell>
                        </TableRow>
